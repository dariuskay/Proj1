...
#         print "returning start state."
    ...
    def getSuccessors(self, state):
        ...
        # Here's a code snippet for figuring out whether a new position hits a wall:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
#             cornersLeft = [state[1][0],state[1][1],state[1][2],state[1][3]]
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                cornersLeft = state[1:5][:]
                if (nextx, nexty) in cornersLeft:
#                     print "reached corner: %r, %r" % (nextx, nexty)
                    cornersLeft.remove((nextx, nexty))
                nextState = [(nextx, nexty)]
                for each in cornersLeft:
...
def cornersHeuristic(state, problem):
    ...
    distances = []
    i = 1
    maxIndex = len(state[1:]) + 1
    while (i < maxIndex):
#         print "manhattan for... " + (str(state[0]))
#         print "and " + str(state[i])
        distance = util.manhattanDistance(state[0], state[i])
